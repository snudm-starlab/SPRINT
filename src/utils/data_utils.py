...
from transformers import (AutoTokenizer, 
                          LlamaTokenizer, )
from utils.general_utils import UnImplementedError
...
# Train Encoders
def get_wikitext2_trainenc(seed, nsamples, seqlen, model, tokenizer, batchify=False):
    """
    Get WikiText2 dataset
    """
...
def get_c4_trainenc(seed, nsamples, seqlen, model, tokenizer, batchify=False):
    """
    Get C4 dataset
    """
    traindata = load_dataset(
...
def get_trainloaders(name, nsamples=128, seed=0, seqlen=2048, model='', batchify=False):
    """
    Get trainloaders
    """
    tokenizer = get_tokenizer(model)
    if 'wikitext2' in name:
        return get_wikitext2_trainenc(seed, nsamples, seqlen, model, tokenizer, batchify=batchify)
    if 'c4' in name:
        return get_c4_trainenc(seed, nsamples, seqlen, model, tokenizer, batchify=batchify)